...
//    TFile * TmpOutFile = new TFile(Form("%s/%s",Path.Data(), fOutFileName.Data()),"recreate");
    Printf("opened output file, writing %lld entries",TmpTree -> GetEntries("(((((((theta_pq < 25)&&(0.62 < p_over_q && p_over_q < 0.96))&&(0.3 < Pmiss.P() && Pmiss.P() < 1.0))&&(1.2 <= Xb))&&(Mmiss < 1.1))&&(2 <= Np))&&(-24.5 < pVertex[0].Z() && pVertex[0].Z() < -20) &&(0.35 < Prec.P()  &&  (-24.5 < pVertex[1].Z() && pVertex[1].Z() < -20))) "));
